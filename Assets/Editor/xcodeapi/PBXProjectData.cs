    ...
    internal class PBXProjectData
    {
        ...
        void RefreshMapsForGroupChildren(string projectPath, string realPath, PBXSourceTree realPathTree, PBXGroupData parent)
        {
            ...
            foreach (string guid in children)
            {
                ...
                if (fileRef != null)
                {
                    pPath = UtilsPBX.CombinePaths(projectPath, fileRef.name);
                    UtilsPBX.CombinePaths(realPath, realPathTree, fileRef.path, fileRef.tree, out rPath, out rTree);
                ...
                if (gr != null)
                {
                    pPath = UtilsPBX.CombinePaths(projectPath, gr.name);
                    UtilsPBX.CombinePaths(realPath, realPathTree, gr.path, gr.tree, out rPath, out rTree);
