...
public class TutorialScrollView_Controller : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
	...
	public List<GameObject> expandContent;
	public List<Image> dot;
	public GameObject navBar;
	[Tooltip("Button to go to the expand page (optional)")]
	public GameObject btnQuestion;
	[Tooltip("Holder sprBg and lblTuttorial (optional)")]
	public GameObject expandHeader;
	[Tooltip("Holder all child of the content (optional)")]
	public GameObject contentHolder;
	public GameObject expandHolder;
	public GameObject HelpSettings;
	[Tooltip("For reset position of scroll view in expand content (optional)")]
	public List<ScrollRect> myScrollRect;
	...
	void Start() {
		...
		item [0].transform.localScale = new Vector3(1.5f,1.5f,1);

		for (int i = 1; i < item.Count; i++) {
			item [i].color = Color.grey;
		}
...
//		if (btnQuestion)
//			btnQuestion.GetComponent<Button> ().onClick.AddListener (() => {onClickBtnQuestion ();});
//		}
	}
...
//				childPosition = new Vector2(i * width - containerWidth / 2 + offsetX, 0f);
				childPosition = new Vector2(i * 210, 0f);
	...
	public void LerpToPage(int aPageIndex) {
		...
		for (int i = 0; i < title.Count; i++) {
			if (i == aPageIndex) {
				...
				item [i].transform.localScale = new Vector3(1.5f,1.5f,1);
				if (isClick) {
					expandContent [i].SetActive (true);
				} else {
				...
				if (isClick) {
					expandContent [i].SetActive (false);
				} else {
					content [i].SetActive (false);
				}
				item[i].color = Color.grey;
				item [i].transform.localScale = new Vector3(1f,1f,1);
			}
		}

		//Reset position
		for (int i = 0; i < myScrollRect.Count; i++) {
			myScrollRect [i].horizontalNormalizedPosition = 0f;
		}
...
//					image.GetComponent<Renderer> ().material.color = Color.white;
	...
	public void onClickBtnQuestion(){
		isClick = true;
		sprPopup1.SetActive (true);
		iTween.MoveTo (myScroll, iTween.Hash("y", 374, "islocal", true, "time",1));
		iTween.MoveTo (myDot, iTween.Hash("y", 141, "islocal", true, "time",1));
		//make the content invisible
		contentHolder.SetActive (false);
		expandHolder.SetActive (true);
		//Display header of expand content
		expandHeader.SetActive (true);
		//Reset position
		for (int i = 0; i < myScrollRect.Count; i++) {
			myScrollRect [i].horizontalNormalizedPosition = 1f;
		}
		//Display expand content
		for (int i = 0; i < expandContent.Count; i++) {
			if (i == _currentPage) {
				expandContent [i].SetActive (true);
			} else {
				expandContent [i].SetActive (false);
	...
	public void onClickBtnPrev(){
		isClick = false;
		sprPopup1.SetActive (false);
		iTween.MoveTo (myScroll, iTween.Hash("y", 419, "islocal", true, "time",1));
		iTween.MoveTo (myDot, iTween.Hash("y", 191, "islocal", true, "time",1));
		//Make the header of expand content invisible
		expandHeader.SetActive(false);
		expandHolder.SetActive (false);
		for (int i = 0; i < expandContent.Count; i++) {
			expandContent [i].SetActive (false);
		}
		//Display the content
